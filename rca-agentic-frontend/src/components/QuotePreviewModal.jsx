              ...
              <table className="w-full border-collapse text-left">
                ...
                <tbody>
                  <tr className="divide-x divide-slate-50">
                    ...
                    <td className="px-6 py-5 text-sm font-black text-indigo-600 text-right">${totalContractValue.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                  </tr>
                </tbody>
              </table>
            </div>
                  ...
                  {lines.map((line, idx) => {
                    ...
                    return (
                      <tr key={idx} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-sm font-bold text-slate-500 text-right">${(line.UnitPrice || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                        <td className="px-6 py-4 text-sm font-bold text-slate-900 text-center">{(line.Quantity || 0).toFixed(2)}</td>
                        <td className="px-6 py-4 text-sm font-bold text-slate-500 text-right">${subtotal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                        <td className="px-6 py-4 text-sm font-black text-indigo-600 text-center">{line.Discount ? `${line.Discount.toFixed(2)}%` : '0.00%'}</td>
                        <td className="px-6 py-4 text-sm font-black text-slate-900 text-right">${(line.TotalPrice || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                      </tr>
                    );
                  })}
                  {lines.length === 0 && (
