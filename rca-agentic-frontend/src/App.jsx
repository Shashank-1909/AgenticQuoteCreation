...
import {
  ...
  Search, FileText, Network, ArrowLeft, Paperclip, X
} from 'lucide-react';
...
const OrchestratorView = ({ onBack, selectedModule, isDark = false }) => {
        ...
        switch (data.type) {
          ...
          case 'TOOL_RESULT':
            ...
            try {
              ...
              if (['search_catalog', 'search_products', 'parse_transcript_to_requirements', 'parse_requirements_doc', 'map_requirements_to_catalog'].includes(data.tool) && parsed.results) {
  ...
  const handleFileUpload = async (e) => {
    ...
    setIsUploading(true);
    setMessages(prev => [...prev, { id: Date.now(), role: 'user', content: `Uploading file: ${file.name}...` }]);
    ...
    try {
      ...
      if (data.status === 'success') {
        setMessages(prev => [...prev, { id: Date.now(), role: 'assistant', content: `File "${file.name}" uploaded and parsed successfully. Processing content...` }]);
        
        // Trigger agent flow with the parsed text
        const promptText = `I have uploaded a requirements document named "${file.name}". Please analyze the following content and extract requirements to create a quote:\n\n${data.text}`;
        ...
        if (ws.current?.readyState === WebSocket.OPEN) {
          ws.current.send(promptText);
        }
      } else {
        setMessages(prev => [...prev, { id: Date.now(), role: 'assistant', content: `⚠️ Upload failed: ${data.message}` }]);
      }
    } catch (err) {
      setMessages(prev => [...prev, { id: Date.now(), role: 'assistant', content: `⚠️ Connection error during upload: ${err.message}` }]);
    } finally {
            ...
            {leftWidth > 110 && messages.map(msg => (
              <div key={msg.id} className="animate-in fade-in slide-in-from-bottom-2">
                ...
                <div className={`p-5 rounded-2xl text-[11px] leading-relaxed transition-all ${
                  ...
                  {msg.content}
                  {msg.role === 'assistant' && (msg.content.toLowerCase().includes('upload') || msg.content.toLowerCase().includes('document')) && (
                    <button
                      onClick={() => fileInputRef.current?.click()}
                      className="mt-4 w-full py-2.5 px-4 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-indigo-500 hover:bg-indigo-500/20 transition-all flex items-center justify-center gap-2 group cursor-pointer"
                    >
                      <Paperclip size={14} className="group-hover:rotate-12 transition-transform" />
                      <span className="text-[9px] font-black uppercase tracking-widest">Click here to upload document</span>
                    </button>
                  )}
                </div>
              </div>
            ...
            <form onSubmit={handleSend} className="group">
              <div className="relative flex items-center">
                ...
                <button
                  ...
                  className="absolute left-2.5 p-2.5 text-slate-400 hover:text-indigo-600 transition-colors z-20 flex items-center justify-center"
                >
                ...
                <input
                  ...
                  disabled={isBusy || isUploading}
                  className="w-full bg-[var(--site-bg)] dark:bg-black/20 border border-slate-200 dark:border-white/5 rounded-2xl py-4 pl-12 pr-12 text-[11px] font-medium focus:border-indigo-500/50 outline-none text-[var(--text-main)] placeholder-slate-400 dark:placeholder-slate-800 transition-all relative z-10 shadow-inner"
                />
