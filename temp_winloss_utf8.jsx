...
const AgentforceView = ({ onBack, selectedModule, isDark = false, setIsDark }) => {
  ...
  const handleWsMessage = (data) => {
    switch (data.type) {
      ...
      case 'QUOTE_UPDATED':
        // Quote modification complete ΓÇö set flag to show preview recommendation after the final reply
        if (data.quote_id) {
      ...
      case 'ERROR':
        addMessage({ type: 'text', content: `ΓÜá∩╕Å ${data.data}` });
        setReasoning(null);
        break;
    }
  };
  ...
  const handleSend = async (e, overrideText = null) => {
    ...
    // Deal history intent ΓÇô intercept before WebSocket ONLY for explicit deal history requests
      ...
      if (detectedAcc) {
        ...
        if (data.status === 'success') {
          ...
          const quotesText = data.quotes.map(q => {
            ...
            return `Quote: ${q.quoteNumber || q.id}, Name: ${q.name}, Status: ${q.status}, Amount: $${q.grandTotal}, Discount: ${q.discount}%, Opportunity: ${q.opportunityName || 'ΓÇö'}, Line Items: [${items}]`;
          }).join('\n');
  ...
  const handleDealHistory = async (accountName = 'Edge Communications', switchView = true) => {
    ...
    try {
      ...
      if (data.status === 'success') {
        setDealHistoryData(data.quotes);
        // Inject AI message about the results ΓÇö addMessage guards against duplicates
        const count = data.quoteCount || data.quotes?.length || 0;
        addMessage({
          type: 'dealHistorySummary',
          content: `Pulled up ${count} historical quote${count !== 1 ? 's' : ''} for ${data.accountName}. Each has been fetched with full line item detail ΓÇö you can view the breakdown on the left or ask me to summarize the patterns across all of them.`,
          accountName: data.accountName,
          quoteCount: count,
        });
      } else {
        addMessage({ type: 'text', content: `Γ¥î Could not fetch deal history: ${data.message}` });
      }
    } catch (err) {
      console.error(err);
      addMessage({ type: 'text', content: 'Γ¥î Failed to fetch deal history. Please try again.' });
    } finally {
      setDealHistoryLoading(false);
      dealHistoryLoadingRef.current = false;
    }
  ...
  return (
    <div className={`agentforce-container ${isDark ? 'dark' : ''} ${config.theme === 'Meta' ? 'meta-theme' : ''}`}>
      ...
      {/* LEFT WORKSPACE ΓÇö CONTEXT VIEW */}
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <span className="text-[10px] font-black uppercase text-emerald-500 tracking-widest">{previewData.records?.[0]?.QuoteNumber} ΓÇö {previewData.records?.[0]?.Status}</span>
                        </div>
                      </div>
                      <button
                        onClick={() => {
                        ...
                        <table className="w-full text-left">
                          ...
                          <tbody>
                            <tr className="text-sm font-bold border-b border-white/5">
                              <td className="px-6 py-6">{previewData.records?.[0]?.Account?.Name || 'ΓÇö'}</td>
                              <td className="px-6 py-6">{previewData.records?.[0]?.Opportunity?.Name || 'ΓÇö'}</td>
      ...
      {/* RIGHT SIDEBAR ΓÇö AGENT INTELLIGENCE */}
      {/* RESIZER HANDLE */}
      <div
